  ...
  add_prediction(prediction_data) {
    ...
    if (this.show_full_state) {
      ...
      for (let index = 0; index < t_fit.length; index++) {
        for (
          let state_index = 0;
          state_index < this.full_state_traces_fit.length;
          state_index++
        ) {
          this.full_state_traces_fit[state_index]["x"].push(
            new Date(t_fit[index] * 1e-6),
          );
          this.full_state_traces_fit[state_index]["y"].push(
            meat_fit[index][state_index],
          );
        }
      }
      ...
      for (
        let state_index = 0;
        state_index < meat_pred[0].length;
        state_index++
      ) {
      ...
      for (let index = 0; index < t_pred.length; index++) {
        for (
          let state_index = 0;
          state_index < this.full_state_traces_pred.length;
          state_index++
        ) {
          this.full_state_traces_pred[state_index]["x"].push(
            new Date(t_pred[index] * 1e-6),
          );
          this.full_state_traces_pred[state_index]["y"].push(
            meat_pred[index][state_index],
          );
        }
      }
    }
