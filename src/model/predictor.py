from meat_estimator import MeatEstimator
from oven_estimator import OvenEstimator
...
class Predictor():
    ...
    def predict(self, ref_temp, max_dur = 8*10*60*60):
        ...
        for i in range(int(max_dur/self._dt)):
            meat_state.append(self._meat_est._model.next_state(meat_state[-1], oven_temp[-1], self._meat_params[0], self._dt))
